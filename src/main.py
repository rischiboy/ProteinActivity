...
"""
def create_model(num_units, reg_lambda, dropout_rate, optimizer, input_dim=104, initial_bias=False):
    if initial_bias:
        model = Sequential([
                Dense(units=num_units,input_shape=(input_dim,),activation='relu',kernel_initializer='random_normal',kernel_regularizer=l2(reg_lambda)),
                Dropout(rate=dropout_rate),
                Dense(units=num_units,activation='relu',kernel_initializer='random_normal',kernel_regularizer=l2(reg_lambda)),
                Dropout(rate=dropout_rate),
...
def Cross_validate_model(X_data,y_data):
    ...
    model_param = dict(batch_size=batch_sizes, optimizer=optimizers, num_units=num_units, dropout_rate=dropout_rate, reg_lambda=reg_lambda)
    f1_scorer = make_scorer(f1_score)

    cv_model = KerasClassifier(build_fn=create_model,epochs=epochs)
    grid = GridSearchCV(estimator=cv_model,param_grid=model_param,n_jobs=2,cv=5,scoring=f1_scorer)
...
def Deep_NN(X_data, y_data, label_count, model_temp_file, best_model_files, curr_model_files, model_param):
    ...
    global network_bias
    network_bias = keras.initializers.Constant(network_bias_const)                              #Custom bias for the neural network
    
    input_dim = len(X_data[0])
    early_stop = EarlyStopping(monitor='loss',mode='min', patience=15)
    checkpoint = ModelCheckpoint(model_temp_file, monitor='val_f1', mode='max', save_best_only=True,save_weights_only=True)
    ...
    if(not os.path.isfile(best_model_files['attributes'])):
        ...
        best_f1_score = scores_df['f1'].values.tolist()[0]                                       #Get the f1 score of the best model if exists
    ...
    for train_idx, val_idx in stf_kfold.split(X_data,y_data):                                   #Train different models on different train-test splits and evaluate them
        ...
        class_weight = {0: w_0, 1: w_1}                                                         #Custom weights to compensate for class imbalance ==> Leads to bad results
        #class_weight = {0: 1, 1: bin_count[0]/bin_count[1]}

        model = create_model(num_units=num_units,reg_lambda=reg_lambda,dropout_rate=dropout_rate,optimizer=optimizer,input_dim=input_dim,initial_bias=bias)

        if weighted:
            model.fit(X_train,y_train,epochs=epochs,batch_size=batch_size,callbacks=[early_stop,checkpoint],validation_data=(X_val,y_val),class_weight=class_weight)
        else:
            model.fit(X_train,y_train,epochs=epochs,batch_size=batch_size,callbacks=[early_stop,checkpoint],validation_data=(X_val,y_val))

        trained_model = create_model(num_units=num_units,reg_lambda=reg_lambda,dropout_rate=dropout_rate,optimizer=optimizer)
        ...
        model_scores_df = model_scores_df.append(dict((m,sc) for m,sc in zip(model.metrics_names,scores)),ignore_index=True)
        cv_counter += 1   
        
    model_scores_df.to_csv(curr_model_files['scores'], index=False, float_format='%.5f')

    best_avg_model,overall_best_f1_score = get_best_model(models_cv,X_data,y_data,val_idx_per_split)
    ...
    curr_model_attr_df.to_csv(curr_model_files['attributes'],index=False,float_format='%.5f')

    best_model = create_model(num_units=num_units,reg_lambda=reg_lambda,dropout_rate=dropout_rate,optimizer=optimizer)
...
"""
def Protein_Encoding(train_file,test_file):
    ...
    X_train, X_test = list(map(lambda l: list(l), X_train)), list(map(lambda l: list(l), X_test))
    y_train = np.array(train_data['Active'].values.tolist())

    one_hot_charged = OneHotEncoder(categories=4*[CHARGED_AA],handle_unknown='ignore')
    ...
    charged_acids_train, charged_acids_test = list(map(lambda l: list(l), charged_acids_train)), list(map(lambda l: list(l), charged_acids_test))
...
#def Oversample(train_data,sampling_factor=0.2):
    ...
    oversampler = imblearn.over_sampling.RandomOverSampler(sampling_strategy=sampling_factor)
...
def main():
    ...
    best_model_files = {'model': best_model_file, 'attributes': best_model_attr_file, 'scores': best_model_scores_file}
    curr_model_files = {'model': curr_model_file, 'attributes': curr_model_attr_file, 'scores': curr_model_scores_file}

    model_param = {'num_units':100, 'reg_lambda':0.0, 'dropout_rate':0.3, 'optimizer':'adam', 'epochs':250, 'batch_size':500, 'cv_splits': 5,'weighted': False, 'bias':True}

    X_train, y_train, X_test = Protein_Encoding(train_file,test_file)
    label_count = np.bincount(y_train)
    
    best_model, new_model = Deep_NN(X_train, y_train, label_count, model_temp_file, best_model_files, curr_model_files, model_param)
    ...
    if new_model:
        ...
        curr_model = create_model(num_units=model_param['num_units'],reg_lambda=model_param['reg_lambda'],dropout_rate=model_param['dropout_rate'],optimizer=model_param['optimizer'])
